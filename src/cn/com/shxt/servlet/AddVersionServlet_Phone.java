...
public class AddVersionServlet_Phone extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if(flag > 0){
			...
			request.getRequestDispatcher("iphone/addVersion.jsp?answer=yes").forward(request, response);
		}else{
			info ="工作日报保存失败！";
			request.setAttribute("info", info);
			request.getRequestDispatcher("iphone/addVersion.jsp?answer=no").forward(request, response);
		}
