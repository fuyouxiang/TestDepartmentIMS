...
public class JXAddBanBenTestServlet extends HttpServlet {
...
/**********************************************************************************************************************/	
		...
		if(type.equals("1")) {
			...
			String sql = "insert into SYS_JIXIAOTEST_SQ (D_ID,D_BUMEN,D_KBOSS,D_KBOSSEMAIL,D_KAIFA,D_DATE,D_CONTENT,D_BIAOZHUN,D_KEMAIL,D_TYPE,D_WEINAME,D_VERSION,D_WIKI,D_ISSQL,D_SQL,D_CONFIG) values " +
					"('" + D_ID + "','" + Bumen + "','" + Boss + "','" + BossEmail + "','" + kaifa + "','" + date + "','" + content + "','" + biaozhun + "','" + k_email + "','" + D_TYPE + "','" + weiServer + "','" + banbenNo + "','" + wiki + "','" + D_ISSQL + "','" + D_SQL + "','" + D_CONFIG + "')";		
			
			int flag = dbutil.update(sql);
			System.out.println(timelog+"添加版本测试申请："+sql);	
			
			String sql2 = "insert into SYS_TESTSQ_LOG (D_ID,T_PEOPLE,T_TIME,T_CAOZUO,T_BEIZHU) values ((select d_id from SYS_JIXIAOTEST_SQ  where D_DATE='" + date + "'),'" + kaifa + "','" + date + "','提交申请', '"+"【内容】:"+content+"【标准】:"+biaozhun+"【构造内容附件】："+wiki+"；"+"【SQL附件】："+D_SQL+"；"+"【配置文件附件】："+D_CONFIG+"')";
			...
			try {
				...
				request.getRequestDispatcher("JixiaoTest/TestFormSubmit_banben.jsp?answer=no").forward(request, response);
			}
		
			if(flag > 0 ){
				request.getRequestDispatcher("JixiaoTest/TestFormSubmit_banben.jsp?answer=yes").forward(request, response);
			}else{
				request.getRequestDispatcher("JixiaoTest/TestFormSubmit_banben.jsp?answer=no").forward(request, response);
			}
		}
			...
			//查询原数据
			String OldIDSql="select D_WIKI,D_SQL,D_CONFIG from SYS_JIXIAOTEST_SQ where D_ID='"+old_id+"'";
			try {
			//如果没有上传新附件的话，用原来的附件
			if(wiki.equals("无")){
