...
<form action="<%=path %>/selectEvectionServlet"  name="MyPageForm" method="post"  id="pageForm" >
          ...
          <td height="62" background="images/nav04.gif">
		   ...
		   <table width="98%" border="0" align="center" cellpadding="0" cellspacing="0">
		    <tr>
			  ...
			  <form action="<%=path %>/selectEvectionServlet2" name="MyPageForm" method="post"  id =formId id ="pageForm" onsubmit="return submitMyForm()">
...
<% 
 ...
 %> 
			    ...
			    <td align="right"><input style="font-size:15px;font-weight:bold;width:80px" size="12" type="button" value="分页显示" onclick="window.location.href='<%=path %>/selectEvectionServlet'"></td>
			  <!-- <td width="144" align="left"><button style="font-size:15px;font-weight:bold;width:100px"  onclick="javascript:exportToExcel('testList','');">导出全部</button></td>	 -->
			  <td width="144" align="left"><button style="font-size:15px;font-weight:bold;width:100px"  onclick="tableToExcel('tableAll','全部出差情况表');">导出全部</button></td>
		    </tr>
          ...
          <table width="100%" border="0" align="left" cellpadding="0" cellspacing="0" id="tableAll">
              <tr>
				...
				<table id = "testList" width="100%" border="2" cellpadding="4" cellspacing="1" bgcolor="#EEEEEE" class="newfont03">
				 <tr class="CTitle" >
                    	<td id="div_title" height="22" colspan="9" align="center" style="font-size:16px">出差情况表</td>
                  </tr>
                  ...
                  <!-- <td height="40" class="bor_1"><input name='isBuy'  type="checkbox"  id="all"  onclick="checkAll(this.checked)"/></td> -->
                    ...
                    <td style="font-size:15px;font-weight:bold">出差月份</td>
                    <td style="font-size:15px;font-weight:bold">出发日期</td>
                    <td style="font-size:15px;font-weight:bold">结束日期</td>
                    <td style="font-size:15px;font-weight:bold">出差地点</td>
                    <td style="font-size:15px;font-weight:bold">任务说明</td>
                    <td style="font-size:15px;font-weight:bold">工期（人/天）</td>
                   ...
                   <tbody>
                        ...
                        <%
                          ...
                          <tr align="center">
                          	...
                          	<td width=40 class="bor_2" style="font-size:15px;text-align:center;font-weight:bold"> <%=i+1 %> </td>
                            <td width=80 class="bor_2" style="font-size:15px;text-align:center;font-weight:bold"><%=stuMap.get("E_NAME") %></td>
                            <td width=50 class="bor_2" style="font-size:15px;text-align:center;font-weight:bold"><%=stuMap.get("E_MONTH") %>月</td>
                            <td width=120 class="bor_2" style="font-size:15px;text-align:center;font-weight:bold"><%=stuMap.get("E_STARTDATE") %></td>
                            <td width=120 class="bor_2" style="font-size:15px;text-align:center;font-weight:bold"><%=stuMap.get("E_ENDDATE") %></td>
                            <td width=80 class="bor_2" style="font-size:15px;text-align:center;font-weight:bold"><%=stuMap.get("E_PLACE") %></td>
                            <td width=300 class="bor_2" style="font-size:15px;text-align:center;font-weight:bold"><%=stuMap.get("E_REMARK") %></td>
                            <td width=80 class="bor_2" style="font-size:15px;text-align:center;font-weight:bold"><%=stuMap.get("E_WORKTIME") %>人/天</td>
                            <!--<td class="bor_2" style="font-size:15px;text-align:center;font-weight:bold"><%=stuMap.get("E_DEPARTMENT") %></td>-->
							<td width=80 class="bor_2 bor_4" style="font-size:15px;text-align:center;font-weight:bold"> 
							<input type="hidden" name="E_ID" value="<%=stuMap.get("E_ID") %>"/>
							<%-- <input name="Submit" type="submit"  value="修改"  onclick = "updateStu(<%=stuMap.get("S_ID") %>"/> --%>  
							<!--<span><img  src="<%=path %>/images/del.gif" width="16" height="16" onclick = "if(window.confirm('您确定要删除吗？')){window.location.href='DeleteEvectionServlet?id=<%=stuMap.get("E_ID") %>&name=<%=stuMap.get("E_NAME") %>&time=<%=stuMap.get("E_WORKTIME") %>'}"/>删除</span>-->
							<button onclick = "if(window.confirm('您确定要删除吗？')){window.location.href='DeleteEvectionServlet?id=<%=stuMap.get("E_ID") %>&name=<%=stuMap.get("E_NAME") %>&time=<%=stuMap.get("E_WORKTIME") %>'}">删除</button></td>
                        </tr>
