...
%>			
<body>
<form action="<%=path %>/selectTestCaseFileServlet?FILE_CATALOG=<%=FILE_CATALOG %>"  name="MyPageForm" method="post"  id="pageForm" >
		...
		<div class="top_dingw">
			<div class="top_bg" id="top_bg">
				<div class="top_main">
            		...
            		<input type="button" value="返回" id="fp" style="font-size:15px;font-weight:bold" onclick="location.href='<%=path %>/selectTestCaseLevelServlet'"/>
            		<input id="CHA_NAME" name="CHA_NAME" type="text" size="18"/>
            		<input type="hidden" id="CHA_APPID" name="CHA_APPID" value="003" type="text"/>
		            ...
		            <li>
		                <a href="youzhishi/DownloadPDF.jsp?ATTACH_PATH=<%=stuMap.get("ATTACH_PATH") %>&ATTACH_NAME=<%=stuMap.get("ATTACH_NAME") %>" target="_blank">
		                    ...
		                    <dl>
		                        ...
		                        <dd>评审结果：<b><%=stuMap.get("TC_RESULT") %></b></dd>
		                        <dd> <a class="sel_btn" id="addTestCaseCheck" href="<%=path %>/youzhishi/addTestCaseCheck.jsp?ATTACH_ID=<%=stuMap.get("ATTACH_ID") %>&ATTACH_NAME=<%=stuMap.get("ATTACH_NAME") %>&CREATE_TIME=<%=stuMap.get("CREATE_TIME") %>">用例评审</a></dd>
